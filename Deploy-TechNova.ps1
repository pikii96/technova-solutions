# ==============================================================================
# PROJEKT: TechNova Solutions - Deployment Script
# ==============================================================================

# -----------------------------------------------------------------------------
# GLOBALNE POSTAVKE
# -----------------------------------------------------------------------------
$subId      = "PUTYOURSUBSCRIPTIONIDHERE"  # <-- PROMIJENI u svoj Azure for Students subscription ID
$location   = "PUTYOURLOCATIONHERE"        # <-- PROMIJENI u svoju regiju (npr. "polandcentral")
$rgName     = "TechNova-RG"
...
# Centralni tagovi za sve resurse (governance + cost tracking)
$globalTags = @{
    Project    = "TechNova"
    Env        = "prod"
    Owner      = "student"
...
Add-Content -Path $logFile -Value "  TechNova Solutions - Deployment Log" -Encoding UTF8
...
Write-Host "|  TECHNOVA SOLUTIONS - DEPLOYMENT SCRIPT              |" -ForegroundColor Cyan
Write-Host "|               Azure for Students                     |" -ForegroundColor Cyan
Write-Host "+======================================================+`n" -ForegroundColor Cyan
Write-Log "Log datoteka: $logFile" -Level "INFO" -Color Cyan

# =============================================================================
# 0. PROVJERA SESIJE + PROVIDERI + DEFENDER FOR CLOUD
# =============================================================================
...
Write-Log "" -Level "INFO"
Write-Log "  Defender for Cloud Free tier" -Level "INFO"
...
# =============================================================================
# 2. ENTRA ID GRUPE + RBAC
# =============================================================================
Write-LogStep "[2/13] Entra ID Grupe i RBAC"
try {
    $departments = @(
...
# =============================================================================
Write-LogStep "[4/13] Storage Account"
...
# 5. LOAD BALANCER
# =============================================================================
Write-LogStep "[5/13] Load Balancer"
try {
...
# =============================================================================
# 6. VIRTUALNE MASINE s Managed Identity
# =============================================================================
Write-LogStep "[6/13] Virtualne Masine"
...
# 7. AKS KLUSTER
# =============================================================================
Write-LogStep "[7/13] AKS Kubernetes Cluster"
...
# 8. APP SERVICE
# =============================================================================
Write-LogStep "[8/13] App Service"
try {
    Write-Host ""
    Write-Host "  +- VAZNO ZA STUDENT PRETPLATU --------------------------------------+" -ForegroundColor Yellow
    Write-Host "  |  Basic B1- NE podrzava auto-scale skriptom                         |" -ForegroundColor Yellow
...
#    Entra ID audit + Web App availability test
# =============================================================================
Write-LogStep "[9/13] Monitoring + Entra ID audit + Availability test"
try {
    ...
    $connStr = "InstrumentationKey=$iKey;IngestionEndpoint=https://${location}-0.in.applicationinsights.azure.com/;LiveEndpoint=https://${location}.livediagnostics.monitor.azure.com/"
    ...
    # -------------------------------------------------------------------------
    Write-Log "" -Level "INFO"
    Write-Log "  Entra ID Diagnostic Settings -> Log Analytics" -Level "INFO"
    ...
    # -------------------------------------------------------------------------
    Write-Log "" -Level "INFO"
    Write-Log "  Web App Availability Test (App Insights ping test)" -Level "INFO"
...
# =============================================================================
# 10. VNET FLOW LOGS + TRAFFIC ANALYTICS
# =============================================================================
Write-LogStep "[10/13] VNet Flow Logs + Traffic Analytics"
try {
    ...
    $flowLogName = "technova-vnetflowlog-prod"
...
# =============================================================================
Write-LogStep "[11/13] Azure Monitor Workbook"
try {
    ...
    $workbookContent = @{
        ...
        items = @(
            @{
                type = 1
                content = @{
                    json = "# TechNova Solutions - Operations Dashboard`n`n**Kljucni pokazatelji infrastrukture u realnom vremenu.**`n`n- CPU iskoristenost VM-ova`n- Web App performanse`n- Audit logovi pristupa`n- Mrezne anomalije"
                }
                name = "header"
            },
            @{
...
# =============================================================================
# 12. ALERTI I ACTION GROUP
# =============================================================================
Write-LogStep "[12/13] Alerti i Obavijesti"
...
# -------------------------------------------------------------------------
# Alert za Web App Availability Test failure
# -------------------------------------------------------------------------
Write-Log "" -Level "INFO"
Write-Log "  Alert za Availability Test failure" -Level "INFO"
...
# =============================================================================
# 13. TAG PROPAGATION  (governance + cost tracking)
# =============================================================================
Write-LogStep "[13/13] Tag Propagation - svi resursi (governance)"
try {
...
Write-Host "  NSG             : $nsgName + Flow Logs + Traffic Analytics"
...
Write-Host "  App Insights    : $appInsightsName + Availability Test"
Write-Host "  Log Analytics   : $lawName"
Write-Host "  Workbook        : 'TechNova Operations Dashboard'"
Write-Host "  Defender        : Free tier aktivan (Secure Score)"
Write-Host "  Alerti          : CPU>80%, HTTP 5xx, Availability<80% -> $alertEmail"
Write-Host "  Entra ID Audit  : Diagnostic Settings -> LAW (ako su ovlasti OK)"
...
Write-Host "  NSG Flow Logs + TA     ~`$ 2/mj"
...
Write-Host "  # Verifikacija dodataka:"
...
Add-Content -Path $logFile -Value "" -Encoding UTF8
Add-Content -Path $logFile -Value "================================================================" -Encoding UTF8
Add-Content -Path $logFile -Value "  ZAVRSETAK DEPLOYMENTA: $endTime" -Encoding UTF8
Add-Content -Path $logFile -Value "================================================================" -Encoding UTF8
