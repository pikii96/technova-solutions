...
# Entra ID Diagnostic Settings name
...
Add-Content -Path $logFile -Value "  TechNova Cleanup - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -Encoding UTF8
Add-Content -Path $logFile -Value "================================================================" -Encoding UTF8

Write-Host @"

+======================================================+
|      TECHNOVA SOLUTIONS - CLEANUP                    |
+======================================================+
"@ -ForegroundColor Red
...
Write-Host "    - VNet Flow Logs: $flowLogName" -ForegroundColor Yellow
Write-Host "    - Entra ID grupe: $($entraGroups -join ', ')" -ForegroundColor Yellow
Write-Host "    - Entra ID Diagnostic Settings: $entraDiagName" -ForegroundColor Yellow
...
# Backward compat: provjeri i obrisi legacy NSG Flow Log ako postoji
...
if ($LASTEXITCODE -eq 0) {
    Write-Log "  -> Brisem legacy NSG Flow Log: $legacyFlowLog..." "WARN"
...
# =============================================================================
# KORAK 3 - ENTRA ID DIAGNOSTIC SETTINGS
# Ne brise se s RG-om - mora se brisati REST API-jem
# =============================================================================
Write-Log "---------------------------------------------------------" "INFO"
Write-Log "[3] Entra ID Diagnostic Settings" "STEP"
...
if ($watcherExists -eq "true") {
    ...
    if ($otherRegions) {
        ...
        Write-Log "  -> PRESKACEM brisanje - moras rucno obrisati samo $location watcher:" "WARN"
...
# Provjeri Entra ID Diagnostic Settings
